...
<a href="https://github.com/Mother-FKR/RazerBlade15-Base-Model-Hackintosh_macOS_Big_Sur/releases/tag/v1.4.0"> <img src="https://img.shields.io/github/v/release/Mother-FKR/RazerBlade15-Base-Model-Hackintosh_macOS_Big_Sur"/> </a>
...
稳定版采用 `OpenCore 0.6.5`，测试版EFI采用 `OpenCore 0.6.6(给我一些时间）`。
...
- **EFI** 
  - `OpenCore` 版本的EFI，版本 `0.6.5`，对应系统 `macOS 11` & `macOS 10.15.6`。较为稳定的EFI，只有在确认稳定后才会进行更新，因此更新频率不是很高。
  - EFI with `OpenCore` , `v0.6.5`, for `macOS 11` & `macOS 10.15.6`. `Stable Version` of EFI, Only update while it's stable to use. Thus not too high freq.
  - **问题：**
    - **睡眠唤醒后时间仍定格在睡眠前（需手动更新时间），确认为RTC问题，短期无时间去解决，有空的小伙伴希望可以帮帮我5555** 
    - **无法自动切换音频输出，开机默认耳机输出，变更输出需要手动切换。(无法解决)**
    - **Type-c口目前部分无法识别，比如苹果的Type-c to lighting 的线连上我的iPhone，电脑无法识别出来，需使用USB-A口才行**
  - **Issue:**
    ...
    - **Might have some connection problem on Type-c**
