...
- **EFI beta**  (使用时记得改名为 `EFI`)
  ...
  - **问题：**
    - **使用 `SecureBoot` 无法进入`恢复模式`**  
    - **包含`EFI`里所有的BUG**
    - **可能会有其他未知BUG**
  - **Issue:**
    - **Can't enter `Recovery` with `SecureBoot`.**
...
## 8/19 更新
这次官方推迟了一天发布 `beta5`，版本号 `20a5354i`，正常 `OTA` 更新即可。增量包大小 `7.15GB`，全量包大小 `12.13GB`。目前版本已无什么大的BUG，更多的为UI BUG，可以日常使用。 `Public Beta2` 内容与 `beta5` 一致，用哪个看你自己了。 

版本说明：  
`20A5354j` ➡️ `Public Beta2`   
`20A5354i` ➡️ `Beta5`
